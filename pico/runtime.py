...
class Pico:
    ...
    def ask(self, user_message):
        ...
        这里就是最关键的入口。
        """
        _debug = bool(os.environ.get("PICO_DEBUG"))
        run_started_at = time.monotonic()
        self.memory.set_task_summary(user_message)
        self.record({"role": "user", "content": user_message, "created_at": now()})
        if _debug:
            print(f"\n{'='*72}\n🟢 [新请求] ask(): {user_message!r}\n{'='*72}")
        ...
        while tool_steps < self.max_steps and attempts < max_attempts:
            attempts += 1
            if _debug:
                print(f"\n──────── 第 {attempts} 轮（已真正执行工具 {tool_steps} 次）────────")
            task_state.record_attempt()
            self.run_store.write_task_state(task_state)
            prompt_started_at = time.monotonic()
            prompt, prompt_metadata = self._build_prompt_and_metadata(user_message)
            if _debug:
                print(f"\n📤 [发给 LLM 的完整 prompt] 共 {len(prompt)} 字符，缓存键={prompt_metadata.get('prompt_cache_key')}")
                print("-" * 72 + "\n" + prompt + "\n" + "-" * 72)
            self.emit_trace(
                task_state,
                "prompt_built",
            ...
            if kind == "tool":
                ...
                if _debug:
                    _m = self.memory.to_dict()
                    print(f"   工具结果(截断):\n{clip(result, 400)}")
                    print(f"   📝 [记忆现状] recent_files={_m['working']['recent_files']} | file_summaries={list(_m['file_summaries'])} | episodic_notes={len(_m['episodic_notes'])}条")
                self.record(
                    {
                        "role": "tool",
                        "name": name,
